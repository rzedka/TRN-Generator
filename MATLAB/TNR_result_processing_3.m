...
% Trying the same as TNR_result_processing.m but with MATLAB pseudorandom
% sequences.

% Conclusion:  MATLAB randn() generates much better and faster random data
% according to their autocorrelation and covariance properties compared to
% my HW TRN generator based on thermal noise.

% Pros: 
% - very fast data generation
% - reproducibility
% - no need for external HW and problems realted..


% Cons: 
% - reproducibility (not true random data)
% - covariance is > 1e-03
% 

% The main con of HW TRN generator is colored noise. Generated noise is
% high-pass and low-pass filtered due to stability of the opamp cascade.
% I believe that with quality opamps I can craft a multichannel TRN 
% generator that produces multiple absolutely independent and almost-identically
% distributed (variances will probably differ, as will the DC bias)

% If the output noise was white, I could generate multiple uncorrelated
% normal variables just by picking the samples of the original stream. 
% Any filtering drastically increases the correlation coefficient "rho".
...
for i=1:N_c
    s_mat(i,:) = M(i:N_c:(N_end-(N_c-i))); % (1 x N_sym)
